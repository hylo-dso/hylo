...
#PRELOAD="singularity exec --bind $SCRATCH/hylo-public/hylo:$HOME,$SCRATCH/cifar10 --nv /home/m/mmehride/bmu/.scinet/conda/envs/ppc64le/hylo/image " # $SCRATCH/AD-v0/test_init.sif "

PRELOAD="module load anaconda3 ; "
PRELOAD+="source /scinet/conda/etc/profile.d/scinet-conda.sh ;"
PRELOAD+="scinet-conda activate hylo ;"

# Args of training script
CMD="main-dist-classification.py --freq 13 --dataset cifar10 --data-dir $SCRATCH/cifar10 --batch-size 128 --model resnet32 --epochs 10 --milestone 35 75 90 --lr 1.8 --lr-decay 0.3 --damping 1.5 --target-damping 0.1 --weight-decay 0.00045 --nproc-per-node 4 --checkpoint-freq 1 --adaptive 10 35 40 --compression-ratio 0.1 --grad-error --enable-is $@"
